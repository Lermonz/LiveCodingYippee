...
const chordsArp = n(chords.add(7).slow(4))
  .scale(mode)
  .release("0.06")
  .arp("[0 1 2 3]*12")
...
const glitch = n("<0 7> 7 21 7 4 <7 21!2> 7 14".fast(2).sometimesBy(0.23, x=>x.clip(0.5).ply("<6 3 4 6>*5")))
  .scale("F4:dorian")
  .vib("160:11.8")
  .postgain(0.79)
.s("square")
...
const pad = n(chords)
  ...
  .lpf(1000)
  .room(0.5)
  //.phaser(sine.range(0,8).slow(2)) //AFTER EVIL
...
//BEAT
const beatD = arrange([7, beats[1]], //0=>1 at 10
          [1, beats[2]])
  .s()
  .speed(rand.range(0.97,1.03))
  //.distort("4.5:0.155") //AT EVIL //REVERT AFTER EVIL
  //.sometimesBy(0.13, x=>x.clip(0.5).ply("<2 3 4 <2 6>>*6")) //AT EVIL2 .13 -> .23 -> .3
  .bpf("2000:0.25")
  .bank("akailinn")
.postgain(1.9)
...
const sections = [
  ...
  stack(pad,bass,beatD,ride,bouncy,chordsArp),
  stack(pad,bass,beatBeuc,ride,bouncy,wind),
  stack(pad,arpeggio,evil,beatD,ride,glitch),
  stack(pad,arpeggio,evil,beatD,ride,glitch,wind,bouncy),
  stack(pad,arpeggio,evil,beatD,glitch,wind,arptriFUCKEDUP)
]
///////////////////INTRO/////CHORUS////EVIL//////BOUNCY///////EVIL2///////////OUTRO
$: pick(sections, "<0@16 1@8 2@16 3@16 4@16 5@16 6@8 7@16 8@8 9@16 10@16 11@8 0@8 ~@64>")
...
//EVIL
//EVIL2 (first 5 lines of default + this v)
  // .hue(H("<0.001 0.18>/2"))
  // .modulateRotate(noise(H("<4 0>/2".fast(1.11)))) //LEAVE THESE 2 LINES AFTER EVIL2
  // .diff(shape(5,H("<1.2 0.9>/2".fast(1.11)))
  //       .modulateScale(noise(H("<60 0>/2".fast(1.11)),H("<2 0>/2".fast(1.11))))
  //       .pixelate(H("<64 [15 <5!2 55> 8 25 9 4 12 <8 88> 3 17 7 18]*2>/2".fast(1.11)),H("<64 [5 8 5 11 <4 74> 12 8 7 16 <8 50> 30 9 7]*2>/2".fast(1.11)))
  //       .hue(0.0001))
//EVIL2
//.add(noise(20,0.15).color(0.35,0.08,0,1).modulateScale(osc(20)).kaleid(50)) //ADD AT EVIL
.out(o0)
all(x=>x.fast(1.11))
```